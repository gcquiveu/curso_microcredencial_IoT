 ...
 * v2: Evita tener dos GAP concurrentes.
...
void InitBLE() {
  ...
  pGeneralAccess = nullptr;
  // Intentamos obtenero el servicio "creado" automáticamente.
  pGeneralAccess = pServer->getServiceByUUID(GENERIC_ACCESS_UUID);
  // Si ha sido creado, pGeneralAccess será  distinto de null y modificamos sus valores.
  if (pGeneralAccess != nullptr) {
    // Mismo proceso con la charcteristic
    BLECharacteristic *DeviceNameCharacteristicOri = nullptr;
    DeviceNameCharacteristicOri = pGeneralAccess->getCharacteristic(BLEUUID((uint16_t)0x2A00));
    if (DeviceNameCharacteristicOri != nullptr) {
      DeviceNameCharacteristicOri->setValue("ESP32-2");
      DeviceNameCharacteristicOri->setWriteProperty(true);
    }
    else{
      pGeneralAccess->addCharacteristic(&DeviceNameCharacteristic);
      DeviceNameCharacteristic.setValue("ESP32-2");
    }
    Serial.println("GAP existia");
  // Si pGeneralAccess es null, creamos el sercicio.
  } else {
    Serial.println("GAP Creado");
